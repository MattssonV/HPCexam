...
void fill_matrix(star_t * array, float_t **matrix, int size, int pad){
    ...
    for (i=0; i<size+pad; i++)
        for (j=0; j<size+pad; j+=10) {
            ...
            matrix[i][j+3] = g+h;
            g = star_distance(array[i],array[j+4]);
            h = starfunc(array[i],array[j+4]);
            matrix[i][j+4] = g+h;
            g = star_distance(array[i],array[j+5]);
            h = starfunc(array[i],array[j+5]);
            matrix[i][j+5] = g+h;
            g = star_distance(array[i],array[j+6]);
            h = starfunc(array[i],array[j+6]);
            matrix[i][j+6] = g+h;
            g = star_distance(array[i],array[j+7]);
            h = starfunc(array[i],array[j+7]);
            matrix[i][j+7] = g+h;
            g = star_distance(array[i],array[j+8]);
            h = starfunc(array[i],array[j+8]);
            matrix[i][j+8] = g+h;
            g = star_distance(array[i],array[j+9]);
            h = starfunc(array[i],array[j+9]);
            matrix[i][j+9] = g+h;
            //matrix[j+3][i+3] = a+b;
            //if (i==j || (i==1&&j==1)) {
            //    printf("%f %d %d\n",a,i,j);
